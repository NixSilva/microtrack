...
def get_bg():
	...
	if os.path.isfile(filename+'-bg.png'):
		print "Reading Background from bg.png"
		bgtmp = cv.LoadImage(filename+'-bg.png', cv.CV_LOAD_IMAGE_UNCHANGED)
		...
		try:
			...
			cv.SaveImage(filename+'-bg.png', bgtmp)
	return bg, nframes
###############################################################################
...
def show_frame(frame):
	image = cv.LoadImage(argv[1]+'-bg.png', cv.CV_LOAD_IMAGE_UNCHANGED)
...
def get_tracks(bg):
	finished_tracks = []
	if os.path.isfile(argv[1]+'.txt'):
		file = open(argv[1]+'.txt')
		file.readline()
		track = Track()
		track_id_old = 0
		for line in file:
...
def show_tracks(tracks):
	...
	cv.ShowImage('image', image)
	cv.SaveImage(argv[1]+'-tracks.png', image)
	cv.WaitKey(1)
###############################################################################
def stat_tracks(tracks):
	#track_info = []
	for track in tracks:
		#sumx = 0.0
		#sumy = 0.0
	...
	cv.WaitKey(0)
	for track in tracks:
		if track.info[0] < avesize * float(argv[3]):
			track.info[7] = False
		elif track.info[1] < avew * float(argv[4]) or track.info[1] > avew * float(argv[5]):
			track.info[7] = False
	show_tracks(tracks)
	cv.WaitKey(0)
...
###############################################################################
def output_tracks(tracks):
	if not os.path.isfile(argv[1]+'.txt'):
		out_file = open(argv[1]+'.txt','w')
		out_file.write('{0:15s} {1:15s} {2:15s} {3:15s} {4:15s} {5:15s} {6:15s}\n'\
	...
	out_file = open(argv[1]+'-filtered.txt','w')
	out_file.write('{0:15s} {1:15s} {2:15s} {3:15s} {4:15s} {5:15s} {6:15s}\n'\
			.format('id', 'time', 'x(m)', 'y(m)', 'width(m)', 'height(m)', 'angle'))
	track_id = 0
	for track in tracks:
	...
	out_file = open(argv[1]+'-track.txt','w')
...
#	tracks = get_tracks_new(particles)
	...
	return
###############################################################################
print tkFileDialog.askopenfile(mode='r')
exit(0)
run()
#cProfile.run('run()')
